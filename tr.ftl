...
index-tagline = Kompleks fikirleri sunmanın en temiz yolu.
index-tagline-mm = Fikirleriniz kadar mükemmel zihin haritaları oluşturun.
index-signupnow = Şimdi Kaydolun
index-login = Zaten kayıtlı mısınız? Giriş yapın.

index-nav-login = Giriş Yap
index-nav-pricing = Fiyatlandırma
index-nav-features = Özellikler
index-nav-gallery = Galeri
index-nav-home = Ana Menü

index-unleash = Yaratıcılığını ortaya çıkar.
index-blurb = Hızlıca ve kolayca not al. Üzerinde beraber çalışmak için arkadaşlarınla ve ekibinle paylaş.
index-viewgallery = Veya ilham için <a data-l10n-name='gallery'>Coggle Gallery</a> 'e göz at.
